...
const submitForm = async () => {
  ...
  let a: any
  if (result) {
    a = await store.createUser(formData)

    if (a.id) {
      $toast.success('Registered Successful. Redirecting to Login')
      setTimeout(() => {
        router.push('/')
      }, 3000)
    } else {
  ...
  <main class="flex flex-row justify-between p-5 w-full h-[100vh] bg-black">
    ...
    <div class="w-[50%] flex justify-between items-center p-5">
      <div class="mx-auto w-[60%]">
        <div><h1 class="font-bold text-3xl mb-5 text-white">Sign Up</h1></div>
        <form @submit.prevent="submitForm">
          ...
          <p class="text-white mt-3 text-center">
            ...
            <RouterLink class="font-bold" to="/"> Login </RouterLink>
          </p>
        </form>
      </div>
    </div>
